...
namespace mntsiwake
{
    class errCheck
    {
        ...
        ///---------------------------------------------------------
        /// <summary>
        ///     決算日付チェック：勘定奉行i10  2017/07/25 </summary>
        /// <param name="iX">
        ///     伝票配列データ添え字</param>
        /// <param name="st">
        ///     伝票配列データ</param>
        ...
        ///------------------------------------------------------
        /// <summary>
        ///     会計期間チェック：勘定奉行i10 2017/07/25 </summary>
        /// <param name="iX">
        ///     伝票配列データ添え字</param>
        /// <param name="st">
        ///     伝票配列データ</param>
        ...
        ///-----------------------------------------------------
        /// <summary>
        ///     部門コードチェック：勘定奉行i10 2017/07/24 </summary>
        /// <param name="iX">
        ///     伝票配列データ添え字</param>
        /// <param name="st">
        ///     伝票配列データ</param>
        ...
        /// <summary>
        ///     税率チェック
        ///     ：勘定奉行i10 2017/08/31 </summary>
        /// <param name="iX">
        ///     伝票配列データ添え字</param>
        /// <param name="st">
        ///     伝票配列データ</param>
        ///--------------------------------------------------
        public void ChkTaxRTNEW(int iX, Entity.InputRecord[] st)
        {
            //行ループ
            for (int i = 0; i < global.MAXGYOU; i++)
            {
                ...
                {
                    //借方
                    if (!ChkTaxRtIndi(st[iX].Gyou[i].Kari.TaxRT))
                    {
                        //エラーテーブルに値を格納
                        ErrCnt++;
                        ErrorTableSet(iX, i, "借", st[iX].Gyou[i].Kari.TaxRT, "不正な税率です", MultiRow.DP_KARI_ZEIRT);
                    }
                    //貸方
                    if (!ChkTaxRtIndi(st[iX].Gyou[i].Kashi.TaxRT))
                    {
                        //エラーテーブルに値を格納
                        ErrCnt++;
                        ErrorTableSet(iX, i, "貸", st[iX].Gyou[i].Kashi.TaxRT, "不正な税率です", MultiRow.DP_KASHI_ZEIRT);
                    }
                }
            }
        }
        ...
        ///---------------------------------------------------
        /// <summary>
        ///     科目コードチェック：勘定奉行i10 2017/07/24</summary>
        /// <param name="Kamoku">
        ///     科目コード</param>
        /// <returns>
        ...
        ///-----------------------------------------------------------
        /// <summary>
        ///     補助コードチェック ：勘定奉行i10 2017/07/24</summary>
        /// <param name="Hojo">
        ///     補助コード</param>
        /// <param name="Kamoku">
        ///     勘定科目コード</param>
        ...
        ///     ok:true,NG:false</returns>
        ///---------------------------------------------
        private Boolean ChkTaxRtIndi(string TaxRT)
        {
            //税率なしのときはOK
            if (TaxRT == string.Empty) return true;

            //数字以外、2桁以上は×
            if (utility.NumericCheck(TaxRT) == false || TaxRT.Length > 2) return false;
