...
namespace mntsiwake
{
    ///-----------------------------------------------------------------------
    /// <summary>
    ///     マスオ綜合事務管理センター 勘定奉行ｉ10版：2017/07/24 </summary>
    ///-----------------------------------------------------------------------
    public partial class Base : Form
    {
        ...
        private void Base_Load(object sender, EventArgs e)
        {
            ...
            global.WorkDir = Properties.Settings.Default.instPath;  // 2017/07/24
            if (global.WorkDir == "")
            {
        ...
        private void DataGetGyou(int iX, string readbuf, Entity.InputRecord[] sDenpyo)
        {
            ...
            // 税率:2017/08/31
            sDenpyo[iX].Gyou[i].Kari.TaxRT = string.Empty;
            ...
            // 税率:2017/08/31
            sDenpyo[iX].Gyou[i].Kashi.TaxRT = string.Empty;
        ...
        private errCheck.Errtbl[] ChkMainNew(Entity.InputRecord[] dData, out Boolean x1)
        {
            ...
            for (int i = 0; i < global.pblDenNum; i++)
            {
                ...
                frmP.progressValue = 83;
                frmP.ProgressStep();
                ec.ChkTaxKbnNEW(i, dData);

                // 税率チェック：2017/08/31
                frmP.Text = "ＮＧチェック中・・・税率";
                frmP.progressValue = 88;
                frmP.ProgressStep();
                ec.ChkTaxRTNEW(i, dData);
        ...
        private void DataShow(int iX, Entity.InputRecord[] st, errCheck.Errtbl[] err)
        {
            ...
            for (int i = 0; i < global.MAXGYOU; i++)
            {
                ...
                gcMultiRow1[i, MultiRow.DP_KARI_ZEIRT].Value = st[iX].Gyou[i].Kari.TaxRT;       //税率 2017/08/31
                ...
                gcMultiRow1[i, MultiRow.DP_KASHI_ZEIRT].Value = st[iX].Gyou[i].Kashi.TaxRT;     //税率 2017/08/31
        ...
        private void ShowNG_Grid(DataGridView tempDGV, errCheck.Errtbl[] err)
        {
            ...
            tempDGV.Columns[2].Width = 55;
            tempDGV.Columns[3].Width = 140;
            tempDGV.Columns[4].Width = 250;
                        ...
                        if (st[iX].Gyou[i].Torikeshi == "0")
                        {
                                ...
                                st[iX].Gyou[i].CopyChk != string.Empty || st[iX].Gyou[i].Tekiyou != string.Empty) 
                            {
                                ...
                                if (iniFlg == true)
                                {
                                    ...
                                    wrkOutputData += Entity.OutPutHeader.kr09 + ",";    // 2017/08/31
                                    wrkOutputData += Entity.OutPutHeader.ks09 + ",";    // 2017/08/31
                                    wrkOutputData += Entity.OutPutHeader.kr10 + ",";    // 2019/09/25
                                    wrkOutputData += Entity.OutPutHeader.ks10;          // 2019/09/25
        ...
        private void InitOutRec(Entity.OutputRecord OutData)
        {
            ...
            OutData.Kari.TaxRT = string.Empty;  // 2017/08/31
            OutData.Kashi.TaxRT = string.Empty; // 2017/08/31
        }

        ///-------------------------------------------------------------
        /// <summary>
        ///     出力データ作成：2017/07/25 勘定奉行i10 </summary>
        /// <param name="iX">
        ///     伝票添え字</param>
        /// <param name="i">
        ///     行添え字</param>
        ...
        ///     出力データ文字列</returns>
        ///-------------------------------------------------------------
        private string SetDataNew(int iX, int i, Entity.InputRecord[] st, Boolean fFlg, Entity.OutputRecord OutData)
        {
            string KEIGEN_88 = "88";        // 軽減８％変換用入力税率
            string KEIGEN_8 = "8";          // 軽減８％
            string ZEISHU_STANDARD = "0";   // 税率種別（0：標準）
            string ZEISHU_KEIGEN = "1";     // 税率種別（1：軽減）
            ...
            // 貸方税率種別：2019/09/25
            string sZeiShubetsu = string.Empty;

            // 借方税率 : 2017/08/31
            if (st[iX].Gyou[i].Kari.TaxRT.Trim() == KEIGEN_88)
            {
                // [88]のとき軽減税率８％とする : 2019/09/25
                OutData.Kari.TaxRT = KEIGEN_8;                      // 税率
                OutData.Kari.TaxRTShubetsu = ZEISHU_KEIGEN;         // 税率種別(1：軽減）
            }
            else
            {
                // その他は経過措置 : 2019/09/25
                OutData.Kari.TaxRT = st[iX].Gyou[i].Kari.TaxRT;     // 税率
                OutData.Kari.TaxRTShubetsu = ZEISHU_STANDARD;       // 税率種別(0：標準）
            }

            // 貸方税率 : 2017/08/31
            if (st[iX].Gyou[i].Kashi.TaxRT.Trim() == KEIGEN_88)
            {
                // [88]のとき軽減税率８％とする : 2019/09/25
                OutData.Kashi.TaxRT = KEIGEN_8;                     // 税率
                OutData.Kashi.TaxRTShubetsu = ZEISHU_KEIGEN;        // 税率種別(1：軽減）
            }
            else
            {
                // その他は経過措置 : 2019/09/25
                OutData.Kashi.TaxRT = st[iX].Gyou[i].Kashi.TaxRT;   // 税率
                OutData.Kashi.TaxRTShubetsu = ZEISHU_STANDARD;      // 税率種別(0：標準）
            }
            ...
            sb.Append(OutData.Tekiyou).Append(",");             // 2017/08/31
            sb.Append(OutData.Kari.TaxRT).Append(",");          // 2017/08/31
            sb.Append(OutData.Kashi.TaxRT).Append(",");         // 2017/08/31
            sb.Append(OutData.Kari.TaxRTShubetsu).Append(",");  // 2019/09/25
            sb.Append(OutData.Kashi.TaxRTShubetsu);             // 2019/09/25
        ...
        private void gcMultiRow1_CellValueChanged(object sender, GrapeCity.Win.MultiRow.CellEventArgs e)
        {
            ...
            switch (e.CellName)
	        {
                ...
                case "txtZeiRT_K":   // 借方税率：2017/09/05
                ...
                case "txtZeiRT_S":   // 貸方税率：2017/09/05
        ...
        private void DlgDataGet()
        {
            ...
            for (int i = 0; i < global.MAXGYOU; i++)
            {
                ...
                DenData[DenIndex].Gyou[i].Kari.TaxRT = gcMultiRow1.GetValue(i, MultiRow.DP_KARI_ZEIRT).ToString();      // 2017/08/31
                ...
                DenData[DenIndex].Gyou[i].Kashi.TaxRT = gcMultiRow1.GetValue(i, MultiRow.DP_KASHI_ZEIRT).ToString();    // 2017/08/31
        ...
        ///----------------------------------------------------
        /// <summary>
        ///     コードの桁数数分左にゼロ埋めした文字列を返す </summary>
        /// <param name="sCode">
        ///     コードの値</param>
        /// <param name="sLength">
        ///     桁長</param>
